...
void Employee::addAge(int a){ 
	employeeAge += a;
	cout << a << " years were added to " << getName() << endl;
}
...
bool Employee::operator==(Employee & e) { return this->phoneNum == e.getPhone(); }

void Employee::operator+(int i)
{
	setAge(getAge() + i);
	cout << i << " years were added to " << getName() << endl;
}
...
ostream & operator<<(ostream & output, Employee & employee)
{
	...
	output << "EmpName: " << employee.getName() << " EmpId : " << employee.getID() << " EmpPhoneNum: " << employee.getPhone() << " EmpAge: " << employee.getAge() << " EmpGender: "
		<< employee.getGender() << "\nEmpJobTitle: " << employee.getTitle() << " EmpSalary: " << employee.getSalary() << " EmpHiredDate: " << employee.getDate();
...
{
	input >> employee.employeeId >> employee.phoneNum;
	return input;
}